...
export default function EditProductPage() {
  ...
  useEffect(() => {
    if (isHydrated && (!user || user.role !== 'seller')) {
      router.push('/');
    } else if (isHydrated && params.slug) {
      fetchProduct();
      fetchCategories();
    }
  }, [user, isHydrated, router, params.slug]);
  ...
  const fetchProduct = async () => {
    try {
      ...
      if (product.thumbnail_url || product.image_url) {
        setImagePreview(product.thumbnail_url || product.image_url);
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (imageFile) {
        ...
        const uploadResult = await uploadProductImageWithThumbnail(imageFile, params.slug);
      ...
      // Update product
      await productsAPI.update(params.slug, productData);
      ...
      showToast.error(
        error.response?.data?.detail || error.message || 'Failed to update product',
        'Error'
      );
    } finally {
      setSaving(false);
